...
<div class="max-w-3xl">
  <form action="{{ route('izin.store') }}" method="POST" enctype="multipart/form-data"
    ...
    <div>
      ...
      <p class="text-xs text-slate-500">Isi data dengan benar. Lampiran (surat dokter/dll) opsional.</p>
    </div>
    ...
    <div class="grid sm:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <input type="date" name="tgl_mulai" id="tgl_mulai" class="mt-1 w-full rounded-lg border-slate-300" required>
      </div>
      <div>
        <label class="text-sm font-medium">Tanggal Selesai</label>
        <input type="date" name="tgl_selesai" id="tgl_selesai" class="mt-1 w-full rounded-lg border-slate-300" required>
      </div>
    </div>

    <div>
      <label class="text-sm font-medium">Keterangan</label>
      <textarea name="keterangan" rows="3" class="mt-1 w-full rounded-lg border-slate-300" placeholder="Contoh: Demam tinggi, istirahat & konsultasi dokter."></textarea>
      <p id="durasiText" class="text-xs text-slate-500 mt-1">Durasi: -</p>
    </div>

    <div>
      <label class="text-sm font-medium">Lampiran (jpg/png/pdf, maks 2MB) — opsional</label>
      <input type="file" name="lampiran" accept=".jpg,.jpeg,.png,.pdf" class="mt-1 block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-slate-900 file:text-white hover:file:bg-slate-700">
    </div>
...
<script>
  ...
  function updateDurasi() {
    ...
    if (days > 0) dur.textContent = `Durasi: ${days} hari`; else dur.textContent = 'Range tanggal tidak valid';
  }
