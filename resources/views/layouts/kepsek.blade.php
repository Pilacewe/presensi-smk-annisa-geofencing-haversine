...
<head>
  ...
  <title>@yield('title','Dashboard Kepsek') — Presensi</title>
...
@php
  ...
  $avatar = $u?->avatar_path
      ...
      : 'https://ui-avatars.com/api/?name='.urlencode($u?->name ?? 'Kepsek').'&background=6366F1&color=fff&size=160';
...
<body class="bg-slate-100 text-slate-800 antialiased">
  ...
  <div class="min-h-screen flex flex-col">
    ...
    {{-- ================= DESKTOP NAVBAR (text only + underline active) ================= --}}
    <header class="hidden md:block sticky top-0 z-40 border-b border-slate-200 bg-white/80 backdrop-blur">
      <div class="mx-auto max-w-7xl px-4">
        <div class="h-16 flex items-center justify-between gap-4">

          {{-- Brand --}}
          <a href="{{ route('kepsek.dashboard') }}" class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-600 to-violet-600 grid place-items-center text-white font-bold">K</div>
          ...
          <nav class="flex items-center gap-1">
            ...
            {!! $link(route('kepsek.dashboard'),        'kepsek.dashboard',        'Dashboard') !!}
            {!! $link(route('kepsek.rekap.harian'),    'kepsek.rekap.harian',     'Rekap Harian') !!}
            {!! $link(route('kepsek.rekap.bulanan'),   'kepsek.rekap.bulanan',    'Rekap Bulanan') !!}
            {!! $link(route('kepsek.account'),         'kepsek.account',          'Account') !!}
          </nav>

          {{-- User kanan --}}
          <div class="flex items-center gap-3">
            <div class="hidden sm:block text-right leading-4">
              <p class="text-[11px] text-slate-500">{{ $u?->jabatan ?? 'Kepala Sekolah' }}</p>
    ...
    <div class="md:hidden sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
      <div class="h-14 px-4 flex items-center justify-between">
        ...
        <div class="text-sm font-semibold truncate">@yield('title','Dashboard Kepsek')</div>
    ...
    <aside id="sidebar"
      ...
      <div class="px-5 h-16 flex items-center gap-3 border-b border-slate-200/70">
        <div class="w-10 h-10 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 grid place-items-center text-white font-bold">K</div>
        <div class="leading-tight">
      ...
      <nav class="px-3 py-4 space-y-1">
        ...
        {!! $m(route('kepsek.dashboard'),        'kepsek.dashboard',        'Dashboard') !!}
        {!! $m(route('kepsek.rekap.harian'),    'kepsek.rekap.harian',     'Rekap Harian') !!}
        {!! $m(route('kepsek.rekap.bulanan'),   'kepsek.rekap.bulanan',    'Rekap Bulanan') !!}
        {!! $m(route('kepsek.account'),         'kepsek.account',          'Account') !!}
        ...
        <div class="mt-3 px-4">
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
              ...
              <p class="text-xs text-slate-500 truncate">{{ $u?->jabatan ?? 'Kepala Sekolah' }}</p>
            </div>
          </div>
    ...
    <section class="hidden md:block w-full bg-gradient-to-r from-slate-50 to-transparent border-b border-slate-200/70">
      <div class="mx-auto max-w-7xl px-4 py-6 flex items-center justify-between gap-3">
        <div>
          <h1 class="text-xl font-semibold">@yield('title','Dashboard Kepsek')</h1>
          @hasSection('subtitle')
