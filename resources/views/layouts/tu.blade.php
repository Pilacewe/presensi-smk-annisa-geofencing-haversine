...
<head>
  ...
  <title>@yield('title','Dashboard TU') — Presensi</title>
...
<body class="bg-slate-100 text-slate-800 antialiased">
  ...
  <div class="min-h-screen flex flex-col">
    ...
    <header class="hidden md:block sticky top-0 z-40 border-b border-slate-200 bg-white/80 backdrop-blur">
      <div class="mx-auto max-w-7xl px-4">
        <div class="h-16 flex items-center justify-between gap-4">
          ...
          {{-- Brand --}}
          <a href="{{ route('tu.dashboard') }}" class="flex items-center gap-3">
          ...
          <nav class="flex items-center gap-1">
            ...
            @endphp
            {!! $link(route('tu.absensi.index'), ['tu.absensi.*'], 'Presensi TU') !!}
          ...
          <div class="flex items-center gap-3">
            <div class="hidden sm:block text-right leading-4">
              <p class="text-[11px] text-slate-500">{{ $u?->jabatan ?? 'Tata Usaha' }}</p>
    ...
    {{-- ================= MOBILE TOPBAR + DRAWER (biarkan seperti di guru) ================= --}}
    <div class="md:hidden sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
      <div class="h-14 px-4 flex items-center justify-between">
        ...
        <div class="text-sm font-semibold truncate">@yield('title','Dashboard TU')</div>
    ...
    <aside id="sidebar"
      ...
      <nav class="px-3 py-4 space-y-1">
        ...
        {!! $m(route('tu.absensi.index'), ['tu.absensi.*'], 'Presensi TU') !!}
        ...
        <div class="mt-3 px-4">
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
              ...
              <p class="text-xs text-slate-500 truncate">{{ $u?->jabatan ?? 'Tata Usaha' }}</p>
            </div>
          </div>
    ...
    <section class="hidden md:block w-full bg-gradient-to-r from-slate-50 to-transparent border-b border-slate-200/70">
      <div class="mx-auto max-w-7xl px-4 py-6 flex items-center justify-between gap-3">
        <div>
          <h1 class="text-xl font-semibold">@yield('title','Dashboard TU')</h1>
          @hasSection('subtitle')
